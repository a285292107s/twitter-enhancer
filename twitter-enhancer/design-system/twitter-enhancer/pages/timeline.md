...
| Token | 值 | 说明 |
|-------|-----|------|
| 正文 | 16px / line-height 1.5 | X 默认 15px/1.3125 过紧；宽列（铺满内容区，1440 视口 980px）下更需行距 |
| 用户名 | 15px / 700 | 保持 X 原值，不抢正文焦点 |
| handle · 时间戳 | 15px / secondary | 层级靠颜色与字重，不靠缩小字号 |
| 正文行长 | `max-width: 72ch` | 列宽放大到 980px 后，英文行长会涨到约 110 字符；72ch 兜住可读性上限（中文约 36 字/行，同样舒适）。图片/视频仍铺满整列，只有正文被限宽 |
...
## 3. 布局（右栏隐藏时与 `/i/grok` 一致）

- X 原生三栏的内容区是一个固定盒子：主列 600 + 间距 30 + 右栏 350（1440 视口再留 70 右边缘）。
  `/i/grok` 是 X 自己的「单栏版」该盒子：主列 980、左缘与 `/home` 完全相同、左导航条不动。
- 本脚本隐藏右栏后照抄这个盒子（主列铺满内容区、左缘不动、**不居中**），
  两个 tab 之间切换时主区几何逐像素相同。
- **左导航条不做任何补偿**：X 自己的 fixed 定位本来就与主列左缘对齐（1440 视口 88 =
  363 − 275；1024 视口自动收成 88 宽图标条）。脚本一旦覆盖（钉 width/left/right），
  就变成「/home 由脚本摆、/i/grok 由 X 摆」两套机制，反而更容易不一致。
- 右栏显示时仍按可用空间收敛，最多 800px（重排空间不够，不能再宽）。
- 实测数值/推导见 `src/features/timeline-width.ts` 头部表格（2026-09-08）。
...
- [ ] 宽列（1440 视口 980px）下正文行长不超过 ~72 字符
- [ ] `/home` 与 `/i/grok` 之间切换时主列几何与左导航条位置不变化
