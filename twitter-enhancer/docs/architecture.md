...
- **「含正文」这条判据认不出 X 的长文（Article）**：媒体钳制的宿主是「向上第一个够宽且
  ...
  **祖先链**上却挂着承载整篇正文的容器（实测 158 个子块 / 23177px），被压成预算高度后正文
  ...
  正文是**文档**，不是推文的媒体行。
- **「纯媒体祖先」不能只看有没有 `tweetText`**：X 会把「推文自己的媒体行」与
  「引用卡 / 文章卡」并排放在**同一个容器**里（2026-09-18 实测 946×1054 = 媒体行 946×512
  ＋ 卡片 946×538）—— 卡片里没有 `tweetText`，于是祖先链爬到那个容器，把卡片一起钉成
  438×580（推文高度 1742 → 1268，冷加载时好时坏）。判据是 `hasContentSibling`：兄弟子树里
  带可见文字、或带不属于 `tweetPhoto` / `videoPlayer` 的图片，就算「别的内容」，那一层到此为止；
  同一条媒体行的另一片媒体（轮播逐格渲染）与纯装饰兄弟（翻页按钮）必须放行，否则
  会打断「宿主里从 1 张图变成 2 张图」的增量路径。
